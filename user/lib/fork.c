...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if(r!=0){
		user_panic("syscall_mem_alloc in cow_entry fault for no cow perm.\n");
	}
	/* Step 4: Copy the content of the faulting page at 'va' to 'UCOW'. */
	/* Hint: 'va' may not be aligned to a page! */
	/* Exercise 4.13: Your code here. (4/6) */
	u_long alignedva = ROUNDDOWN(va,BY2PG);
	memcpy((void*)(UCOW),(void*)(alignedva),BY2PG);
	// Step 5: Map the page at 'UCOW' to 'va' with the new 'perm'.
	/* Exercise 4.13: Your code here. (5/6) */
	syscall_mem_map(0,(void*)UCOW,0,(void*)(alignedva),perm);
...
static void duppage(u_int envid, u_int vpn) {
	...
	//if(((perm & PTE_D)==0)||((perm & PTE_COW)!=0)||((perm & PTE_LIBRARY)!=0)){
	//	r=syscall_mem_map(0,(void *)addr,envid,(void *)addr,perm);
	//	if(r!=0){
	//		user_panic("mem_map fault\n");
	//	}
	//} else if ((perm & PTE_D)!=0){
	//	r=syscall_mem_map(0,(void *)addr,envid,(void *)addr,perm|PTE_COW);
	//	if(r!=0){
	//		user_panic("mem_map fault\n");
	//	}
	//	syscall_mem_map(0,(void *)addr,0,(void *)addr,perm|PTE_COW);
	//	if(r!=0){
	//		user_panic("mem_map fault\n");
	//	}
	//}
...
int fork(void) {
	...
	for (u_long j=UTEMP;j<USTACKTOP;j+=BY2PG){
		if (((vpd[j>>PDSHIFT]) & PTE_V) && ((vpt[j>>PGSHIFT]) & PTE_V)){
			duppage(child,VPN(j));
		}
	}
